...
forked by ebs-Uchimura.
edited: 2020/10/12
 
**********************************************************/

// define variables.
var sourceFolder, files, sourceDoc, targetFile, fList;

// Select the source folder.
sourceFolder = Folder.selectDialog('Select the folder with Illustrator files you want to convert to PNG', '~');

// Call main function.
fList = main(sourceFolder, [".ai", ".eps"]);
...
function main(folderObj, ext) {
  ...
  function getFolder(fObj) {
    var fileList = new Array();
    fileList = fObj.getFiles();
    if(fileList.length > 0) {
      for (var i=0; i<fileList.length; i++){
        if (fileList[i].getFiles) {
          getFolder(fileList[i]); // Loop till subfolder exists
        } else {
